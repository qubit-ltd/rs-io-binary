...
macro_rules! zig_zag_write_method {
    ($doc:literal, $name:ident, $ty:ty) => {
        ...
        {
            async move {
                let mut bytes = [0_u8; ZigZagCodec::<$ty, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];
...
pub trait AsyncZigZagWriteExt: AsyncOutput<Item = u8> {
    zig_zag_write_method!("Asynchronously writes ZigZag `i8`.", write_zig_zag_i8_async, i8);
    zig_zag_write_method!("Asynchronously writes ZigZag `i16`.", write_zig_zag_i16_async, i16);
    zig_zag_write_method!("Asynchronously writes ZigZag `i32`.", write_zig_zag_i32_async, i32);
    zig_zag_write_method!("Asynchronously writes ZigZag `i64`.", write_zig_zag_i64_async, i64);
    zig_zag_write_method!("Asynchronously writes ZigZag `i128`.", write_zig_zag_i128_async, i128);
