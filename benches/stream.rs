...
use qubit_io_binary::{
    ...
    BufferedZigZagWriter, Leb128ReadExt, Leb128Reader, Leb128WriteExt, Leb128Writer, StringReadExt,
    StringWriteExt, ZigZagReadExt, ZigZagReader, ZigZagWriteExt, ZigZagWriter,
};
...
fn write_micro_values_buffered(values: &[u64]) -> Vec<u8> {
    let mut writer =
        BufferedBinaryWriter::<_, LittleEndian>::with_capacity(ChunkedMemoryOutput::new(7), 16);
    for &value in values {
        writer
            .write_u64(value)
...
fn read_micro_values_ext(bytes: &[u8]) -> u64 {
    ...
    values_digest(
        (0..MICRO_VALUE_COUNT).map(|_| input.read_u64_le().expect("extension read should succeed")),
    )
}
...
fn read_micro_values_wrapper(bytes: &[u8]) -> u64 {
    ...
    values_digest(
        (0..MICRO_VALUE_COUNT).map(|_| reader.read_u64().expect("wrapper read should succeed")),
    )
}
...
fn read_micro_values_buffered(bytes: &[u8]) -> u64 {
    ...
    values_digest(
        (0..MICRO_VALUE_COUNT).map(|_| reader.read_u64().expect("buffered read should succeed")),
    )
}
...
fn read_uleb_std_manual_file(path: &Path, fields: &[UlebField]) {
    ...
    for field in fields {
        match *field {
            UlebField::U8(_) => {
                checksum ^= read_uleb_std_manual(&mut reader, u128::from(u8::MAX)).unwrap()
            }
            UlebField::U16(_) => {
                checksum ^= read_uleb_std_manual(&mut reader, u128::from(u16::MAX)).unwrap()
            }
            UlebField::U32(_) => {
                checksum ^= read_uleb_std_manual(&mut reader, u128::from(u32::MAX)).unwrap()
            }
            UlebField::U64(_) => {
                checksum ^= read_uleb_std_manual(&mut reader, u128::from(u64::MAX)).unwrap()
            }
            UlebField::Usize(_) => {
                checksum ^= read_uleb_std_manual(&mut reader, usize::MAX as u128).unwrap()
            }
    ...
    for field in fields {
        let decoded = match *field {
            ...
            ZigZagField::I16(_) => zigzag_decode_std_manual(
                read_uleb_std_manual(&mut reader, u128::from(u16::MAX)).unwrap(),
            ),
            ZigZagField::I32(_) => zigzag_decode_std_manual(
                read_uleb_std_manual(&mut reader, u128::from(u32::MAX)).unwrap(),
            ),
            ZigZagField::I64(_) => zigzag_decode_std_manual(
                read_uleb_std_manual(&mut reader, u128::from(u64::MAX)).unwrap(),
            ),
            ZigZagField::Isize(_) => zigzag_decode_std_manual(
                read_uleb_std_manual(&mut reader, usize::MAX as u128).unwrap(),
            ),
            ZigZagField::I128(_) => {
                zigzag_decode_std_manual(read_uleb_std_manual(&mut reader, u128::MAX).unwrap())
            }
        };
        checksum ^= decoded;
    }
...
fn bench_prod_mixed_binary_pipeline(c: &mut Criterion) {
    ...
    group.bench_function("ext_bufwriter_write", |b| {
        b.iter(|| {
            write_mixed_ext(
                BufWriter::new(File::create(&raw_path).unwrap()),
                black_box(&fields),
            )
        })
    });
