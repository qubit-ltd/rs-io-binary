...
fn test_transcode_decode_input_ext_delegates_public_raw_and_codec_reads() {
    let mut reader =
        BufferedBinaryReader::<_, LittleEndian>::with_capacity(Cursor::new(vec![0x34, 0x12, 0x56, 0x78]), 1);

    assert_eq!(ByteOrder::LittleEndian, reader.byte_order());
    let mut buffer = [0_u8; 1];
    assert_eq!(1, reader.read(&mut buffer).expect("raw read should return one byte"),);
    assert_eq!(0x34, buffer[0]);
    assert_eq!(1, reader.seek_to(SeekFrom::Start(1)).expect("seek should succeed"),);
    assert_eq!(0x5612, reader.read_u16().expect("buffered codec read should succeed"),);
}

#[test]
fn test_transcode_decode_input_ext_maps_incomplete_public_decode() {
    let mut reader = BufferedLeb128Reader::<_, NonStrict>::with_capacity(Cursor::new(vec![0x80]), 1);
