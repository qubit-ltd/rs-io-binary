...
use qubit_io_binary::{NonStrict, Strict, ZigZagCodec, ZigZagReader, ZigZagWriter};
...
fn test_zig_zag_reader_reports_all_instantiated_error_paths() {
    ...
    let mut reader =
        ZigZagReader::<_, NonStrict>::new(Cursor::new(vec![
        0x80u8;
...
fn test_zig_zag_reader_read_and_seek_delegate_to_inner_reader() {
    let mut reader = qubit_io_binary::ZigZagReader::<_, qubit_io_binary::NonStrict>::new(
